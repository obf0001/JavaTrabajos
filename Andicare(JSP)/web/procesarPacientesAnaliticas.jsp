<%-- 
    Document   : cargarPacientesAnaliticas
    Created on : 25-abr-2023, 10:45:57
    Author     : Oday
--%>
...
//Inicializamos las variables para que borren su contenido antes de meter nuevos
        String nombrePac = "";
        String apellidosPac = "";
        String fechaNacPacStr = "0001-01-01";
        String estUsuLibreViewPac = "";
        String NUHSAPac = "";
        String DNIPac = "";
        ...
        nombrePac = request.getParameter("nombrePac");
        apellidosPac = request.getParameter("apellidosPac");
        estUsuLibreViewPac = request.getParameter("estUsuLibreViewPac");
        NUHSAPac = request.getParameter("NUHSAPac");
        DNIPac = request.getParameter("DNIPac");
        ...
        if (request.getParameter("fechaNacPac") != null && !request.getParameter("fechaNacPac").isEmpty()) {
            fechaNacPacStr = request.getParameter("fechaNacPac");
        }
        Date fechaNacPac = java.sql.Date.valueOf(fechaNacPacStr);
        ...
        try {
            ...
            // Crear la sentencia SQL INSERT para agregar los datos a la tabla correspondiente
            String sql = "INSERT INTO pacientes (Nombre, Apellidos, Fecha_Nacimiento, Estado_Usuario_LibreView, NUHSA, DNI, DNI_Medico)"
                    + " VALUES (?, ?, ?, ?, ?, ?, ?);";

            statement = conexion.prepareStatement(sql);

            statement.setString(1, nombrePac);
            statement.setString(2, apellidosPac);
            statement.setDate(3, fechaNacPac);
            statement.setString(4, estUsuLibreViewPac);
            statement.setString(5, NUHSAPac);
            statement.setString(6, DNIPac);
            statement.setString(7, profSanitarioPac);
            ...
            String sql2 = "INSERT INTO analiticas (Glucosa_Promedio, Prom_Vistas_Dias, Porc_Objetivo, Sensor_tiempo_Porc_Act, Sensor_Duracion_Promedio_Glucosa_Baja, "
                    ...
                    + "Eventos_Glucosa_Baja, Prom_Glucosa_Baja_Dia, Ind_Gestion_Glucosa_GMI_Porc, NUHSA, Fecha_Analitica, Tecnico_implicado)"
                    + " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
            ...
            response.sendRedirect("./Paginas/cargaPacientes.jsp");
